var com_github_culmat_jsTreeTable = (function () {
	...
	function treeTable(table) {
		...
		$("tr[data-tt-level]", table).each(function (index, tr) {
			...
			$(tr).find("td:first-child img").on("click", function () {
				$(this).parents("tr").get(0).trChildrenVisible ? $(this).parents("tr").get(0).trCollapse(true) : $(this).parents("tr").get(0).trExpand(true)
			})
		})
		return table
	}
